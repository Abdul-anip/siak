...
/* ------------- KURIKULUM CRUD ------------- */
    ...
    echo "<div class='topbar'><h2>Data Kurikulum</h2><div><a class='btn' href='index.php?page=kurikulum&aksi=tambah'>+ Tambah Kurikulum</a></div></div>";
    echo "<div class='card-content'>";
    if(isset($_GET['aksi']) && $_GET['aksi']=='tambah'){
        ...
        <form method="post" action="index.php?act=kurikulum_add">
            ...
            <label>Tahun (format YYYY)</label><input class="input" name="kurTahun" placeholder="2024" required />
            <label>Nama Kurikulum</label><input class="input" name="kurNama" required />
            <label><input type="checkbox" name="kurIsAktif" checked /> Aktif</label><br><br>
            <button class="btn" name="save_kurikulum" type="submit">Simpan</button>
            <a class="btn" style="background:#777;margin-left:8px" href="index.php?page=kurikulum">Batal</a>
        </form>
        <?php
    } else {
        $res = $koneksi->query("SELECT k.*, p.prodiNama FROM kurikulum k LEFT JOIN program_studi p ON k.kurProdiId = p.prodiId ORDER BY k.kurId");
        echo "<table class='table'><tr><th>#</th><th>Prodi</th><th>Tahun</th><th>Nama Kurikulum</th><th>Aktif</th><th>Aksi</th></tr>";
        $no=1;
        while($r=$res->fetch_assoc()){
            echo "<tr>
                    <td>".($no++)."</td>
                    ...
                    <td>
                        <a class='btn small' style='background:#2d9cdb' href='index.php?page=kurikulum&aksi=edit&id=".h($r['kurId'])."'>Edit</a>
                        <a class='btn small' style='background:#d9534f' href='index.php?act=kurikulum_delete&id=".h($r['kurId'])."' onclick=\"return confirm('Hapus kurikulum ini?')\">Hapus</a>
                    </td>
                  </tr>";
        }
        echo "</table>";
...
/* ------------- MATAKULIAH CRUD ------------- */
    ...
    echo "<div class='topbar'><h2>Data Matakuliah</h2><div><a class='btn' href='index.php?page=matkul&aksi=tambah'>+ Tambah Matakuliah</a></div></div>";
    echo "<div class='card-content'>";
    if(isset($_GET['aksi']) && $_GET['aksi']=='tambah'){
        ...
        <form method="post" action="index.php?act=matkul_add">
            ...
            <label>Kode</label><input class="input" name="mkKode" required />
            <label>Nama</label><input class="input" name="mkNama" required />
            <label>Semester</label><input class="input" name="mkSemester" type="number" min="1" max="12" />
            <label>SKS</label><input class="input" name="mkSks" type="number" min="0" />
            <label><input type="checkbox" name="mkIsAktif" checked /> Aktif</label><br><br>
            <button class="btn" name="save_matkul" type="submit">Simpan</button>
            <a class="btn" style="background:#777;margin-left:8px" href="index.php?page=matkul">Batal</a>
        </form>
        <?php
    } else {
        $res = $koneksi->query("SELECT m.*, k.kurNama FROM matakuliah m LEFT JOIN kurikulum k ON m.mkKurId = k.kurId ORDER BY m.mkId DESC");
        echo "<table class='table'><tr><th>#</th><th>Kode</th><th>Nama</th><th>Kurikulum</th><th>Sem</th><th>SKS</th><th>Aktif</th><th>Aksi</th></tr>";
        $no=1;
        while($r=$res->fetch_assoc()){
            echo "<tr>
                    <td>".($no++)."</td>
                    ...
                    <td>
                        <a class='btn small' style='background:#2d9cdb' href='index.php?page=matkul&aksi=edit&id=".h($r['mkId'])."'>Edit</a>
                        <a class='btn small' style='background:#d9534f' href='index.php?act=matkul_delete&id=".h($r['mkId'])."' onclick=\"return confirm('Hapus matakuliah ini?')\">Hapus</a>
                    </td>
                  </tr>";
        }
        echo "</table>";
...
/* ------------- DOSEN CRUD ------------- */
    ...
    echo "<div class='topbar'><h2>Data Dosen</h2><div><a class='btn' href='index.php?page=dosen&aksi=tambah'>+ Tambah Dosen</a></div></div>";
    echo "<div class='card-content'>";
    if(isset($_GET['aksi']) && $_GET['aksi']=='tambah'){
        $jur = $koneksi->query("SELECT jurId, jurNama FROM jurusan ORDER BY jurNama");
        $prodi = $koneksi->query("SELECT prodiId, prodiNama FROM program_studi ORDER BY prodiNama");
        ...
        <form method="post" action="index.php?act=dosen_add">
            <label>NIDN</label><input class="input" name="dsnNidn" required />
            <label>Nama</label><input class="input" name="dsnNama" required />
            ...
            <a class="btn" style="background:#777;margin-left:8px" href="index.php?page=dosen">Batal</a>
        </form>
        <?php
    } else {
        $r = $koneksi->query("SELECT d.*, j.jurNama, p.prodiNama FROM dosen d LEFT JOIN jurusan j ON d.dsnJurId=j.jurId LEFT JOIN program_studi p ON d.dsnProdiId=p.prodiId ORDER BY d.dsnNama");
        echo "<table class='table'><tr><th>#</th><th>NIDN</th><th>Nama</th><th>Jurusan</th><th>Prodi</th><th>JK</th><th>Aksi</th></tr>";
        $no=1;
        while($row=$r->fetch_assoc()){
            echo "<tr>
                    ...
                    <td>
                        <a class='btn small' style='background:#2d9cdb' href='index.php?page=dosen&aksi=edit&id=".h($row['dsnNidn'])."'>Edit</a>
                        <a class='btn small' style='background:#d9534f' href='index.php?act=dosen_delete&id=".h($row['dsnNidn'])."' onclick=\"return confirm('Hapus dosen ini?')\">Hapus</a>
                    </td>
                  </tr>";
        }
        echo "</table>";
...
/* ------------- MAHASISWA CRUD ------------- */
    ...
    if(isset($_GET['aksi']) && $_GET['aksi']=='tambah'){
        ...
        <form method="post" action="index.php?act=mahasiswa_add">
            <label>NIM</label><input class="input" name="mhsNim" required />
            <label>Nama</label><input class="input" name="mhsNama" required />
            <label>Tgl Lahir</label><input class="input" name="mhsTglLahir" type="date" />
            <label>Jenis Kelamin</label>
            ...
            <label>Jurusan</label>
            <select name="mhsJurId" class="input">
                <option value="0">-- Pilih Jurusan --</option>
                <?php while($j=$jur->fetch_assoc()): ?>
                    <option value="<?=h($j['jurId'])?>"><?=h($j['jurNama'])?></option>
                <?php endwhile; ?>
            ...
            <label>Program Studi</label>
            <select name="mhsProdiId" class="input">
                <option value="0">-- Pilih Prodi --</option>
                <?php while($p=$prodi->fetch_assoc()): ?>
                    <option value="<?=h($p['prodiId'])?>"><?=h($p['prodiNama'])?></option>
                <?php endwhile; ?>
        ...
        $res = $koneksi->query("SELECT m.*, j.jurNama, p.prodiNama FROM mahasiswa m LEFT JOIN jurusan j ON m.mhsJurId=j.jurId LEFT JOIN program_studi p ON m.mhsProdiId=p.prodiId ORDER BY m.mhsNama");
        echo "<table class='table'><tr><th>#</th><th>NIM</th><th>Nama</th><th>Jurusan</th><th>Prodi</th><th>JK</th><th>Aksi</th></tr>";
        $no=1;
        while($r=$res->fetch_assoc()){
            echo "<tr>
                    <td>".($no++)."</td>
                    ...
                    <td>
                        <a class='btn small' style='background:#2d9cdb' href='index.php?page=mahasiswa&aksi=edit&id=".h($r['mhsNim'])."'>Edit</a>
                        <a class='btn small' style='background:#d9534f' href='index.php?act=mahasiswa_delete&id=".h($r['mhsNim'])."' onclick=\"return confirm('Hapus mahasiswa ini?')\">Hapus</a>
                    </td>
                  </tr>";
        }
        echo "</table>";
